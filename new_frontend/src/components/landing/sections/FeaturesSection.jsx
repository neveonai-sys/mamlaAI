...
function FeatureCard({ feature: f }) {
  ...
  const showImage = f.image && !imgFailed;

  return (
    <div className="group relative flex flex-col overflow-hidden rounded-[18px] border border-slate-200 bg-background-light p-6 transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-elevated">
      {showImage && (
        <div className="mb-5 overflow-hidden rounded-xl border border-slate-200 bg-white">
          <img
            src={f.image}
            alt={`Mamla.ai — ${f.title}`}
            loading="lazy"
