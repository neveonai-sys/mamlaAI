...
import { searchSCIOfficeReportByCase, searchSCIOfficeReportByDiary, downloadSCIPdf } from './apiSCI';
...
function ReportRow({ item, onDownload, downloading }) {
  return (
    <div className="rounded-[18px] border border-primary/10 bg-background-light p-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="font-bold text-ink">{item.report_title || 'Office Report'}</p>
          <p className="mt-1 font-mono text-xs text-slate-400">{item.report_date || '—'}</p>
        </div>
        {item.doc_url && (
          <button
            type="button"
...
export default function SCIOfficeReportsTerminal() {
  ...
  async function handleSubmit(e) {
    ...
    setError('');
    setLoading(true);
    dispatch(beginBlocking({ message: 'Searching office reports...' }));

    try {
      let res;
      if (tab === 'case') {
        res = await searchSCIOfficeReportByCase({ case_type: caseType, case_no: caseNo, case_year: caseYear });
      } else {
        res = await searchSCIOfficeReportByDiary({ diary_no: diaryNo, diary_year: diaryYear });
      }
      const data = res.data;
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-4 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-black text-ink">Office Reports</h1>
      </div>
        ...
        <div className="mt-6">
          ...
          {results.length === 0 ? (
            <div className="rounded-[24px] border border-primary/10 bg-white p-6 text-center text-slate-500">
              No office reports found matching your search.
            </div>
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
              {results.map((item, i) => (
                <ReportRow key={i} item={item} onDownload={handleDownload} downloading={downloadingUrl === item.doc_url} />
              ))}
            </div>
          )}
        </div>
