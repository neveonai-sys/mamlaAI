...
export default function LawyerSearch() {
  ...
  const doSearch = useCallback(async (q, p, filters) => {
    ...
    try {
      ...
      setResults(data);
      saveSearchCache('lawyers', q, p, filters, data);
    } catch (err) {
  ...
  const fetchDefaults = useCallback(async () => {
    try {
      const resp = await getEcourtsDefaults('lawyers');
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', px: { xs: 2, md: 3 }, py: 3 }}>
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">Lawyers</Typography>
      </Breadcrumbs>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Search by advocate name to find their cases across all courts.
      </Typography>
      ...
      {isDefault && (
        <Chip
          ...
          label={`Sample advocate directory${defaultRefreshedAt ? ` · Updated ${new Date(defaultRefreshedAt).toLocaleDateString('en-IN')}` : ''} · Search by name to filter`}
          size="small"
          variant="outlined"
          color="primary"
          sx={{ mb: 2 }}
            ...
            <>
              {!isDefault && totalHits > 0 && (
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  {totalHits.toLocaleString()} cases found for advocate &quot;{query}&quot;
                </Typography>
              )}
